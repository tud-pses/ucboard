...
Es stehen zwanzig parametrierbare Messgruppen zur Verfügung, die zum automatischen Verschicken von Messdaten verwendet werden. In diesen können verschiedene Kanäle (Sensorwerte) zusammengefasst werden. Neben den "`normalen"' Kanälen stehen noch die in \tabref{tab:Comm:DAQ:SpecialChannels} aufgeführten "`Sonderkanäle"' zur Verfügung, die Metadaten zu den Datensätzen enthalten.

Gruppe 1 enthält die Werte des Ultraschalls. Es wird gesendet, wenn alle Ultraschallwerte vorliegen, wobei maximal \valunit{10}{ms} nach dem Erfassen des ersten Wertes gewartet wird. Die Daten werden base64-codiert und mit einer CRC16-Prüfsumme verschickt.
\begin{verbatim}
...
Gruppe 3 enthält die Beschleunigungswerte in der Ebene und die Gierrate. Die Daten werden alle \valunit{10}{ms} verschickt. Dabei werden alle innerhalb der \valunit{10}{ms} erfassten Daten gemittelt.
\begin{verbatim}
...
\begin{table}[htbp]%
	...
	\begin{tabular}{lp{10cm}lll}
		...
		{\color[rgb]{0.75,0.75,0.75}\verb|_DLY|} & Zeit zwischen Datensatzerstellung und Versenden & ms & \verb|uint32_t| & 4 \\
		{\color[rgb]{0.75,0.75,0.75}\verb|_DLY16|} & min(\verb|_DLY|, 65\,535) & ms  & \verb|uint16_t| & 2 \\
		{\color[rgb]{0.75,0.75,0.75}\verb|_DLY8|} & min(\verb|_DLY|, 255) & ms & \verb|uint8_t| & 1 \\
		\mybottomrule
	\end{tabular}
\end{table}
